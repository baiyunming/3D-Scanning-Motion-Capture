...
struct ReprojectCost
{
    ...
    template <typename T>
    bool operator()(const T* const extrinsic, const T* const pos3d, T* residuals) const
    {
        const T* r = extrinsic;
...
int main(int argc, char** argv) {
    ...
    //convert extrinsic matrix into 6 DOF
    std::vector<Mat> extrinsic_after_optimization;
    for (int i = 0; i < rgbImages.size() - 2; i++) {
        Mat r;
        Matrix4f extrinsic = pointClouds[i].getCameraExtrinsics();
        cout << extrinsic << endl;
        Mat transform(4, 4, CV_64FC1);
        eigen2cv(extrinsic, transform);
        Mat Rotation(3, 3, CV_64FC1);
        Rotation = transform(cv::Range(0, 3), cv::Range(0, 3));
        Rodrigues(Rotation, r);
        cout << Rotation << endl;
        cout << r << endl;
        Mat translation(3, 1, CV_64FC1);
        translation = transform(cv::Range(0, 3), cv::Range(3, 4));
        cout << translation << endl;
        
        Mat extrinsics(6, 1, CV_64FC1);
        r.copyTo(extrinsics.rowRange(0, 3));
        translation.copyTo(extrinsics.rowRange(3, 6));
      
        extrinsic_after_optimization.push_back(extrinsics);
    }

    //convert landmark float into double 
    vector<Vector3d> global_3D_points_double;
    for (int i = 0; i < global_3D_points.size(); i++) {
        Vector3d intermediate_landmark;
        intermediate_landmark.x() = double(global_3D_points[i].x());
        intermediate_landmark.y() = double(global_3D_points[i].y());
        intermediate_landmark.z() = double(global_3D_points[i].z());
        global_3D_points_double.push_back(intermediate_landmark);
    }
    ...
    for (int i = 0; i < 10; i++) {
        ...
        for (size_t i = 0; i < extrinsic_after_optimization.size(); ++i)
        {
            problem.AddParameterBlock(extrinsic_after_optimization[i].ptr<double>(), 6);
        }
        // fix the first camera.
        problem.SetParameterBlockConstant(extrinsic_after_optimization[0].ptr<double>());
        ...
        for (int j = 0; j < (pointClouds.size() - 2); j++) {
            std::vector<cv::KeyPoint> points2D = pointClouds[j].getPoints2d();
            std::map<int, int> match_2D_to_3D = pointClouds[j].get2d_3d_correspondence();

            for (std::map<int, int>::iterator it = match_2D_to_3D.begin(); it != match_2D_to_3D.end(); ++it) {
                if (global_3D_points_double[it->second][0] == MINF)
                    continue;

                cv::Point2d observation = points2D[it->first].pt;
                ceres::CostFunction* cost_function = new ceres::AutoDiffCostFunction<ReprojectCost, 2, 6, 3>(new ReprojectCost(observation));

                problem.AddResidualBlock(cost_function, loss_function, extrinsic_after_optimization[j].ptr<double>(), &(global_3D_points_double[it->second].x()));
            }
        }
    ...
    std::vector<Matrix4f> optimized_extrinsics;
    for (int i = 0; i < extrinsic_after_optimization.size(); i++) {
        
        Mat Rotation;
        Rodrigues(extrinsic_after_optimization[i](cv::Range(0, 3), cv::Range(0, 1)), Rotation);
        cout << extrinsic_after_optimization[i] << endl;
        cout << extrinsic_after_optimization[i](cv::Range(0, 3), cv::Range(0, 1)) << endl;
        Matrix3f Rotation_Matrix;
        cout << Rotation << endl;
        cv2eigen(Rotation, Rotation_Matrix);
        
        Vector3d translation ;
        translation[0] = extrinsic_after_optimization[i].at<double>(3,0);
        translation[1] = extrinsic_after_optimization[i].at<double>(4,0);
        translation[2] = extrinsic_after_optimization[i].at<double>(5,0);
        cout << translation << endl;

        Vector3f translation_float;
        translation_float[0] = float(translation[0]);
        translation_float[1] = float(translation[1]);
        translation_float[2] = float(translation[2]);
        cout << translation_float << endl;

        Matrix4f extrinsic; 
        extrinsic.setIdentity();
        extrinsic.block<3, 3>(0, 0) = Rotation_Matrix;
        extrinsic.block<3, 1>(0, 3) = translation_float;

        optimized_extrinsics.push_back(extrinsic);
        cout << extrinsic;
    }


    generateOffFile("Optimization.off", global_3D_points, global_color_points, optimized_extrinsics, ground_truth_transformationMatrices);
    generateTxTFile("Optimization.txt", optimized_extrinsics, timestamp);
    
    generateCompareOffFile("OptimizationCompare.off", global_3D_points, global_color_points, estimated_extrinsic, optimized_extrinsics, ground_truth_transformationMatrices);
