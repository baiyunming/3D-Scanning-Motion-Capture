...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		double value = (_x - m_center).squaredNorm() - m_radius* m_radius;
		return value;
	}
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		double value = pow((_x - m_center).squaredNorm() + m_radius* m_radius - m_a*m_a, 2) - 4 * m_radius * m_radius * ((_x[0] - m_center[0])* (_x[0] - m_center[0]) + (_x[1] - m_center[1])* (_x[1] - m_center[1]));
		return value;
	}
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		for (unsigned int i = 0; i < m_numCenters; i++) {
			Vector3d sample_point = m_funcSamp.m_pos[i];
			result = result + m_coefficents[i] * EvalBasis((sample_point - _x).norm());
		}
		

		result = result + m_coefficents[m_numCenters] * _x[0] + m_coefficents[m_numCenters + 1] * _x[1] + m_coefficents[m_numCenters + 2] * _x[2] + m_coefficents[m_numCenters + 3];
		return result;
	}
