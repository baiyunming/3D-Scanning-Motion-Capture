...
struct GaussianCostFunction
{
	...
	bool operator()(const T* const mu, const T* const sigma, T* residual) const
	{
		...
		residual[0] = point.y-((T(1)/sqrt(T(2* EIGEN_PI)* pow(sigma[0],2)))* exp(-pow(point.x-mu[0],2)/(T(2)*pow(sigma[0],2))));
		return true;
	}
...
int main(int argc, char** argv)
{
	...
	// Read data points
	// const std::string file_path = "F:/TUM Learning Material/20WS/Informatik/3D Scanning & Motion Capture/exercise_4/exercise_4_src/data/points_gaussian.txt";
