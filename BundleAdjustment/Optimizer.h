...
//Cost function for all frames, update both the landmark positions and the rotation and translation vector  
...
public:
	...
	void solve_Bundle_Adjustment_partially(std::vector<PointCloud>& pointClouds, std::vector<Vector3f>& global_3D_points, const int starting_frame, const int window_size) {

		double** poses = (double**)malloc(sizeof(double*) * (window_size));
		...
		std::cout << "Starting BA Partially" << std::endl;
		for (int i = 0; i < m_nIterations; i++) {
			...
			std::vector<int> landmark_frequency_inside_window(global_3D_points.size());

			for (int j = starting_frame; j < (starting_frame + window_size); j++) {
				...
				for (std::map<int, int>::iterator it = match_2D_to_3D.begin(); it != match_2D_to_3D.end(); ++it) {
					landmark_frequency_inside_window[it->second] = landmark_frequency_inside_window[it->second] + 1;
				}
			}
			...
			for (int j = starting_frame; j < (starting_frame + window_size) ; j++) {
				...
				Matrix4f estimatedPose = pointClouds[j].getCameraExtrinsics();
				poses[j-starting_frame] = PoseConverter<double>::pose6DOF(estimatedPose);

				prepareConstraints(pointClouds[j], points3D, poses[j-starting_frame], problem, landmark_frequency_inside_window);
				if (j == starting_frame) {
					problem.SetParameterBlockConstant(poses[j - starting_frame]);
				}
			}
			...
			for (int j = starting_frame; j < (starting_frame + window_size); j++) {
				Matrix4f updated_matrix = PoseConverter<double>::convertToMatrix(poses[j-starting_frame]);
				pointClouds[j].setCameraExtrinsics(updated_matrix);
			}
...
private:
	...
	//Constraint preparation for partially solving Bundle adjustment
	//only consider the landmarks which have been observed at least 3 times inside the optimized_window (4) 
	void prepareConstraints(PointCloud& pointCloud, double** global_3D_points, double* pose, ceres::Problem& problem, std::vector<int> frequency_table) const {
		...
		for (std::map<int, int>::iterator it = match_2D_to_3D.begin(); it != match_2D_to_3D.end(); ++it) {
			...
			if (frequency_table[it->second] < 3)
				continue;
