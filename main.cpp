...
std::string DATA_PATH = "F:/TUM Learning Material/20WS/Informatik/3D Scanning & Motion Capture/bundle_adjustment/src/data/rgbd_dataset_freiburg1_xyz/";
...
#define NUM_FEATURES_EACH_FRAME 2000
#define NUM_CONSECUTIVE_FRAMES 2
...
void get_data(std::string file_path, std::vector<cv::Mat>& depth_images, std::vector<cv::Mat>& rgb_images, std::vector<Matrix4f>& transformationMatrices, std::vector<string>&timestamp) {
 ...
 //读取两张图片，每张图片返回num_features个特征点坐标，描述子，良好匹配good_matches
void find_matches(
