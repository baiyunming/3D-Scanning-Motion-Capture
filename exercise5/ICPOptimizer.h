...
public:
	...
	bool operator()(const T* const pose, T* residuals) const {
		...
		poseIncrement.apply(m_source, m_transformed_Point);
...
public:
	...
	bool operator()(const T* const pose, T* residuals) const {
		...
		poseIncrement.apply(m_source, m_transformed_Point);
		residuals[0] = T(m_weight*LAMBDA)*((m_transformed_Point[0]-T(m_targetPoint.x()))*T(m_targetNormal.x())+(m_transformed_Point[1]-T(m_targetPoint.y()))*T(m_targetNormal.y())+(m_transformed_Point[2]-T(m_targetPoint.z()))*T(m_targetNormal.z()));
		...
		for (unsigned i = 0; i < nPoints; ++i) {
			...
			if (match.idx >= 0) {
				...
				// to the Ceres problem.'
				    double* pose = poseIncrement.getData();
					//PointToPointConstraint point2point(sourcePoint, targetPoint, weight);
...
private:
	...
	Matrix4f estimatePosePointToPlane(const std::vector<Vector3f>& sourcePoints, const std::vector<Vector3f>& targetPoints, const std::vector<Vector3f>& targetNormals) {
		...
		for (unsigned i = 0; i < nPoints; i++) {
			...
			A(4 * i, 0) = s.y() * n.z() - s.z() * n.y();
			A(4 * i, 1) = s.z() * n.x() - s.x() * n.z();
			A(4 * i, 2) = s.x() * n.y() - n.x() * s.y();
			A(4 * i, 3) = n.x();
			A(4 * i, 4) = n.y();
			A(4 * i, 5) = n.z();

			b(4 * i) = -n.x() * s.x() - n.y() * s.y() - n.z() * s.z() + d.x() * n.x() + d.y() * n.y() + d.z() * n.z();
